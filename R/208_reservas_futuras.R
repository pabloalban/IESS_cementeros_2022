...
reserva_matematica <- reserva_matematica %>%
  ...
  left_join( ., cementeros %>% dplyr::select( cedula, fecha_jubilacion ), by ='cedula' ) %>%
  ...
  mutate( reserva_matematica_pr = reserva_matematica * pr_acu,
...
reserva_anual<- reserva_matematica %>%
  ...
  dplyr::select( anio,  nuevos_pensionistas, reserva_matematica_pr, montepio_pr, liquidacion_pr, total_pr ) %>%
  arrange( anio )

#Guardar en Rdata-----------------------------------------------------------------------------------
message( '\tGuardando reservas matemáticas' )

save( reserva_matematica,reserva_anual,
