...
## 1.1. Pirámide de afiliados activos en 2022----------------------------------------------------------
message( '\tGraficando piramides poblacionales activa de los cementeros' )

aux <- afi_edad_sexo %>%
  mutate( fdp = if_else( sexo == 'M',-fdp,
                       fdp ) ) %>%
  filter( edad >= 15,
         edad <= 105 ) %>%
  arrange( sexo, edad )
...
lbls_y <-
  paste0( as.character( c( 
    seq( abs( lim_y[1] ), 0,-salto_x ) * 100, seq( salto_x, lim_y[2], salto_x ) *
      100
 ) ), "%" )
...
iess_pir_edad_sexo <-
  ggplot( aux, aes( x = edad, y = fdp, fill = sexo ) ) +
  xlab( 'Edad' ) +
  ylab( '' ) +
  geom_bar( 
    data = aux %>% filter( sexo == 'F' ),
    stat = 'identity',
    colour = "white",
    size = 0.1
 ) +
  geom_bar( 
    data = aux %>% filter( sexo == 'M' ),
    stat = 'identity',
    colour = "white",
    size = 0.1
 ) +
...
## 1.2. Pirámide salario promedio--------------------------------------------------------------------

aux <- sal_edad_sexo  %>%
  mutate( fdp = if_else( sexo == 'M',-salario_promedio ,
                       salario_promedio ) ) %>%
  filter( edad >= 15,
         edad <= 105 ) %>%
  arrange( sexo, edad ) %>%
  mutate( fdp = if_else( fdp > 50000,
                       mean( salario_promedio, na.rm = TRUE ),
                       fdp ) )

salto_y <- 5
brks_y <-
  round( c( seq( min( aux$fdp ), 0, length.out = 5 ), seq( 0, max( aux$fdp ), length.out = 5 )[-1] ) )
lbls_y <-
  paste0( "$",
         formatC( 
           abs( brks_y ),
...
iess_pir_edad_sexo_SD <-
  ggplot( aux, aes( x = x, y = fdp, fill = sexo ) ) +
  xlab( 'Edad' ) +
  ylab( '' ) +
  geom_bar( 
    data = aux[sexo == 'F'],
...
# 3. Pensionistas-----------------------------------------------------------------------------------
## 3.1. Pirámide de pensiones ----------------------------------------------------------------------

aux <- tab_pensiones_edad_sexo  %>%
  mutate( fdp = if_else( sexo == 'M',-pen_prom,
                       pen_prom ) ) %>%
  filter( edad >= 15,
         edad <= 105 ) %>%
  arrange( sexo, edad )
