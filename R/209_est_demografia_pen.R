...
cortes_edad <-
  c( quantile( a$edad, probs = seq( 0, 1, length.out = 9 ) ) )

etiquetas_edad <-
  c( paste0( 
    "( ",
    formatC( 
      cortes_edad[1:length( cortes_edad ) - 1],
      digits = 0,
      format = 'f',
      big.mark = '.',
      decimal.mark = ','
    ...
    " - ",
    formatC( 
      cortes_edad[2:length( cortes_edad )],
      digits = 0,
      format = 'f',
      big.mark = '.',
      decimal.mark = ','
...
aux_t <-
  left_join( aux_h, aux_m, by = "rango_edad" ) %>% select( rango_edad, hombres :=
                                                          afiliados.x, mujeres := afiliados.y )
aux_t <- aux_t %>% mutate( Total = rowSums( aux_t[, 2:3], na.rm = TRUE ) )
aux_total_edad <- aux_t %>%
  add_row( 
    rango_edad = "Total",
    hombres = sum( aux_t$hombres, na.rm = TRUE ),
    mujeres = sum( aux_t$mujeres, na.rm = TRUE ),
    Total = sum( aux_t$Total, na.rm = TRUE )
   )
...
#Tabla de pensionistas administrativos--------------------------------------------------------------

tab_pensionistas_administrativos <- pensiones %>%
  filter( !is.na( ocupacion_plani ) ) %>%
...
#Tabla de afiliados por imposiciones----------------------------------------------------------------
